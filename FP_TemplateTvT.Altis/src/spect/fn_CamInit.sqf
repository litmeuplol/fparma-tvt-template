...
_heliH = "HeliHEmpty" createVehicleLocal [0,0,0];
_heliH setPosATL (getPosATL _unit);
_pos = getPosATL _heliH;
if (_pos isEqualTo [0,0,0]) then {
	if (!isNil "server") then {
		_pos = getPosATL server;	
	}else{
...
f_cam_playersOnly = if (!ismultiplayer) then {false} else{true};
